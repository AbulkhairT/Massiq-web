...
/* ─────────────────────────────────────────────────────────────────────────────
 * Food-scan daily limit
 *
 * Free users get FREE_FOOD_SCANS_PER_DAY photo-food-scans per calendar day.
 * Premium users have unlimited.
 *
 * Counter lives in localStorage, keyed by user ID + ISO date so it resets
 * automatically at midnight without any cron / server involvement.
 * Only counts when the user actually confirms and adds the meal — failed or
 * cancelled scans do NOT count.
 * ───────────────────────────────────────────────────────────────────────────── */

export const FREE_FOOD_SCANS_PER_DAY = 2;

function foodScanDayKey(userId) {
  const today = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
  return `massiq:${userId}:foodScans:${today}`;
}

/**
 * Number of food scans the user has completed today.
 * Returns 0 for unrecognized/absent values.
 */
export function getFoodScansUsedToday(userId) {
  if (!userId || typeof window === 'undefined') return 0;
  try {
    const v = localStorage.getItem(foodScanDayKey(userId));
    return v ? Math.max(0, parseInt(v, 10)) : 0;
  } catch { return 0; }
}

/**
 * Remaining food scans for today.  Returns Infinity for premium users.
 */
export function foodScansRemainingToday(userId, subscription) {
  if (isPremiumActive(subscription)) return Infinity;
  return Math.max(0, FREE_FOOD_SCANS_PER_DAY - getFoodScansUsedToday(userId));
}

/**
 * Returns true when the user is allowed to start a food scan right now.
 */
export function canFoodScan(userId, subscription) {
  if (isPremiumActive(subscription)) return true;
  return getFoodScansUsedToday(userId) < FREE_FOOD_SCANS_PER_DAY;
}

/**
 * Increment the food-scan counter for today.
 * Call ONLY after the user confirms and logs the meal.
 */
export function recordFoodScanUsed(userId) {
  if (!userId) return;
  try {
    const used = getFoodScansUsedToday(userId);
    localStorage.setItem(foodScanDayKey(userId), String(used + 1));
  } catch {}
}
