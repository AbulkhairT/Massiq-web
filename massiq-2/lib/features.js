...
/**
 * Free users get this many food scans total (lifetime).
 * Premium users have no limit.
 */
export const FREE_FOOD_SCAN_LIMIT = 2;

/**
 * Returns the number of food scans used (total, per user).
 * Source of truth: entitlements.free_food_scans_used from DB.
 * localStorage is cache-only for display before entitlements load.
 */
export function getFoodScansUsedTotal(entitlements, userId) {
  if (entitlements != null && Number.isFinite(entitlements.free_food_scans_used)) {
    return entitlements.free_food_scans_used;
  }
  if (!userId) return 0;
  try {
    const key = `miq:food-scans:${userId}`;
 ...
 */
export function setFoodScanCache(userId, used) {
  if (!userId || !Number.isFinite(used)) return;
  try {
    localStorage.setItem(`miq:food-scans:${userId}`, String(used));
  } catch {}
}

/**
 * Returns true if the user can perform a food scan right now.
 * Prefer entitlements from DB; fallback to cache for optimistic UI.
 */
export function canFoodScan(subscription, entitlements, userId) {
  if (isPremiumActive(subscription)) return true;
  const used = getFoodScansUsedTotal(entitlements, userId);
  return used < FREE_FOOD_SCAN_LIMIT;
}

/**
 * Returns the number of free food scans remaining.
 */
export function foodScansRemainingTotal(subscription, entitlements, userId) {
