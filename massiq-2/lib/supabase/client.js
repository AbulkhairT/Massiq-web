...
export async function createScanSignalSet(token, userId, payload) {
  const row = {
    user_id: userId,
    scan_id: payload.scanId,
    body_fat_low: payload.body_fat_low ?? null,
    body_fat_high: payload.body_fat_high ?? null,
    ab_definition_score: payload.ab_definition_score ?? null,
    waist_definition_score: payload.waist_definition_score ?? null,
    chest_score: payload.chest_score ?? null,
    upper_chest_score: payload.upper_chest_score ?? null,
    shoulders_score: payload.shoulders_score ?? null,
    arms_score: payload.arms_score ?? null,
    back_v_taper_score: payload.back_v_taper_score ?? null,
    symmetry_score: payload.symmetry_score ?? null,
    lighting_quality_score: payload.lighting_quality_score ?? null,
    pose_quality_score: payload.pose_quality_score ?? null,
    framing_quality_score: payload.framing_quality_score ?? null,
    mirror_distortion_risk_score: payload.mirror_distortion_risk_score ?? null,
    flex_bias_score: payload.flex_bias_score ?? null,
  ...
  const rows = await supabaseFetch('/rest/v1/scan_signal_sets', {
    method: 'POST',
    headers: { ...authHeaders(token), Prefer: 'return=representation' },
    body: JSON.stringify(row),
  });
...
export async function getRecentScanSignalSets(token, userId, limit = 3) {
  const rows = await supabaseFetch(
    `/rest/v1/scan_signal_sets?user_id=eq.${userId}&select=*&order=created_at.desc&limit=${limit}`,
    { method: 'GET', headers: authHeaders(token) },
  );
  return Array.isArray(rows) ? rows : [];
}

export async function getUserBodyState(token, userId) {
  const rows = await supabaseFetch(
    `/rest/v1/user_body_state?user_id=eq.${userId}&select=*&limit=1`,
    { method: 'GET', headers: authHeaders(token) },
  );
  return Array.isArray(rows) && rows[0] ? rows[0] : null;
}

export async function upsertUserBodyState(token, userId, payload) {
  const row = {
    user_id: userId,
    based_on_scan_id: payload.based_on_scan_id ?? null,
    previous_scan_id: payload.previous_scan_id ?? null,
    stable_body_fat_low: payload.stable_body_fat_low ?? null,
    stable_body_fat_high: payload.stable_body_fat_high ?? null,
    stable_lean_mass_kg: payload.stable_lean_mass_kg ?? null,
    stable_ab_definition_score: payload.stable_ab_definition_score ?? null,
    stable_waist_definition_score: payload.stable_waist_definition_score ?? null,
    stable_chest_score: payload.stable_chest_score ?? null,
    stable_upper_chest_score: payload.stable_upper_chest_score ?? null,
    stable_shoulders_score: payload.stable_shoulders_score ?? null,
    stable_arms_score: payload.stable_arms_score ?? null,
    stable_symmetry_score: payload.stable_symmetry_score ?? null,
    primary_limiting_factor: payload.primary_limiting_factor ?? null,
    secondary_limiting_factors: payload.secondary_limiting_factors ?? [],
    phase_recommendation: payload.phase_recommendation ?? null,
    state_confidence_label: payload.state_confidence_label ?? null,
    state_confidence_score: payload.state_confidence_score ?? null,
    last_meaningful_change_at: payload.last_meaningful_change_at ?? null,
    stabilization_notes: payload.stabilization_notes ?? null,
    state_payload: payload.state_payload ?? {},
    updated_at: payload.updated_at || new Date().toISOString(),
  };
  const rows = await supabaseFetch('/rest/v1/user_body_state?on_conflict=user_id', {
    method: 'POST',
...
export async function createFoodSignalSet(token, userId, payload) {
  const row = {
    user_id: userId,
    food_log_id: payload.food_log_id ?? null,
    food_scan_event_id: payload.food_scan_event_id ?? null,
    identified_items: payload.identified_items ?? [],
    canonical_matches: payload.canonical_matches ?? [],
    portion_estimates: payload.portion_estimates ?? [],
    estimated_calories_low: payload.estimated_calories_low ?? null,
    estimated_calories_high: payload.estimated_calories_high ?? null,
    estimated_protein_low: payload.estimated_protein_low ?? null,
    estimated_protein_high: payload.estimated_protein_high ?? null,
    estimated_carbs_low: payload.estimated_carbs_low ?? null,
    estimated_carbs_high: payload.estimated_carbs_high ?? null,
    estimated_fat_low: payload.estimated_fat_low ?? null,
    estimated_fat_high: payload.estimated_fat_high ?? null,
    ambiguity_flags: payload.ambiguity_flags ?? [],
    confidence_label: payload.confidence_label ?? null,
    confidence_score: payload.confidence_score ?? null,
    signal_payload: payload.signal_payload ?? {},
  };
  const rows = await supabaseFetch('/rest/v1/food_signal_sets', {
    method: 'POST',
    headers: { ...authHeaders(token), Prefer: 'return=representation' },
    body: JSON.stringify(row),
  });
