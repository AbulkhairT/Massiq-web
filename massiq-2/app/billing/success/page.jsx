...
export default function BillingSuccessPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const session = await initializeSession().catch(() => null);
      const token = session?.access_token;
      const userId = session?.user?.id || session?.user_id;

      if (!token || !userId) {
        if (!cancelled) setStage('unauthenticated');
        return;
      }
      ...
      <div style={{ maxWidth: 420, width: '100%', textAlign: 'center' }}>
        ...
        {stage === 'unauthenticated' && (
          <>
            ...
            <div style={{ fontSize: 14, color: C.muted, lineHeight: 1.6, marginBottom: 28 }}>
              We could not restore your local session on this page. Open the app to continue and sign in only if prompted.
            </div>
            <button
