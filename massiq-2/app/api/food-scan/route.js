...
async function getFoodScansRemainingToday(userId) {
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!supabaseUrl || !serviceKey) return FREE_FOOD_SCAN_LIMIT;
  try {
    const res = await fetch(
      `${supabaseUrl}/rest/v1/rpc/food_scans_remaining_today`,
      {
        method: 'POST',
        headers: {
    ...
    if (!res.ok) {
      return FREE_FOOD_SCAN_LIMIT;
    }
    const data = await res.json().catch(() => null);
    const remaining = Number(data);
    return Number.isFinite(remaining) ? remaining : FREE_FOOD_SCAN_LIMIT;
  } catch {
    return FREE_FOOD_SCAN_LIMIT;
  }
}
...
export async function POST(req) {
  ...
  if (!isPremium) {
    const remainingToday = await getFoodScansRemainingToday(userId);
    if (remainingToday <= 0) {
