...
import { hasFeature, isPremiumActive, FEATURES, canScan, scansRemaining, FREE_SCAN_LIMIT, canFoodScan, foodScansRemainingTotal, FREE_FOOD_SCAN_LIMIT, getFoodScansUsedTotal, setFoodScanCache } from '../lib/features';
...
function Paywall({ userId, accessToken, onClose, persistGate, onRefreshPremium, subscription }) {
  const [loading, setLoading] = useState(false);
  const [error,   setError]   = useState('');
  const [checkoutOpened, setCheckoutOpened] = useState(false);
  const refreshInProgressRef = useRef(false);

  // When premium activates while paywall is open (user completed in new tab), close or show success
  useEffect(() => {
    if (checkoutOpened && subscription && isPremiumActive(subscription)) {
      setCheckoutOpened(false);
      onClose?.();
    }
  }, [checkoutOpened, subscription, onClose]);

  const handleRefreshPremium = async () => {
    if (!onRefreshPremium || refreshInProgressRef.current) return;
    ...
    try {
      ...
      setError(err?.message || 'Could not refresh. Try again.');
    } finally {
    ...
    try {
      ...
      const poll = async () => {
        while (attempts < 12) {
          ...
          attempts++;
          try { await onRefreshPremium?.(); } catch {}
        }
      };
      poll();
    } catch (err) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Checkout-opened state: user completes purchase in new tab */}
        {checkoutOpened && (
          <div style={{
            marginBottom: 16, padding: 14, background: 'rgba(114,184,149,0.1)',
            border: '1px solid rgba(114,184,149,0.25)', borderRadius: 12,
          }}>
            <div style={{ fontSize: 14, color: C.white, marginBottom: 8, lineHeight: 1.5 }}>
              Complete your purchase in the opened tab. We&apos;ll unlock premium here once payment is confirmed.
            </div>
            <button
              ...
              className="bp"
              style={{
                width: '100%', background: C.green, color: '#0A0D0A', border: 'none',
                padding: '10px 16px', borderRadius: 99, fontSize: 14, fontWeight: 700,
                cursor: loading ? 'default' : 'pointer', marginBottom: 8,
              }}
            >
              {loading ? 'Checking…' : 'I completed purchase — refresh'}
            </button>
            <div style={{ fontSize: 12, color: C.dimmed, textAlign: 'center' }}>
              Closed the checkout tab? Close this and tap Upgrade again to reopen.
            </div>
          </div>
        )}
...
function HomeTab({ profile, activePlan, setTab, showToast, scanHistory, subscription, entitlements, onUpgrade, userId, accessToken, onFoodScanComplete }) {
  ...
  const handleScanFile = (file) => {
    ...
    if (!canFoodScan(subscription, entitlements, userId)) {
      showToast?.(`Food scan limit reached (${FREE_FOOD_SCAN_LIMIT} free). Upgrade for unlimited scans.`);
      return;
    }
    setScanning(true);
  ...
  return (
    <div className="screen" style={{ paddingBottom: 120 }}>
      ...
      {(() => {
        const foodRemaining = foodScansRemainingTotal(subscription, entitlements, userId);
        const foodUsed      = getFoodScansUsedTotal(entitlements, userId);
          ...
          <>
            {!premium && !limitHit && foodUsed > 0 && (
              <div style={{
                ...
                {foodUsed} / {FREE_FOOD_SCAN_LIMIT} food scans used
              </div>
            )}
            {limitHit && (
              <div style={{
                ...
                <span style={{ fontSize: 11, color: C.muted }}>Food scan limit reached</span>
                <button className="bp" onClick={onUpgrade} style={{
                  background: 'none', border: 'none', cursor: 'pointer',
...
function LogMealModal({ onClose, onAdd, macros, profile, subscription, onUpgrade, userId, accessToken, entitlements, onFoodScanComplete }) {
  ...
  const analyzePhoto = (file) => {
    ...
    if (!canFoodScan(subscription, entitlements, userId)) {
      setError(`Food scan limit reached (${FREE_FOOD_SCAN_LIMIT} free). Upgrade for unlimited scans.`);
      onUpgrade?.();
      return;
    }
    setAnalyzing(true); setError('');
  ...
  const handleAdd = () => {
    ...
    if (fromPhoto) onFoodScanComplete?.();
    onAdd(safeMeal);
    onClose();
  };
          ...
          <div style={{ background: C.cardElevated, borderRadius: 16, padding: 16, marginBottom: 20 }}>
            ...
            {aiTab === 'describe' ? (
              ...
              <div>
                ...
                {canFoodScan(subscription, entitlements, userId) ? (
                  ...
                  <button className="bp" onClick={() => onUpgrade?.()} style={{
                    ...
                    Food scan limit reached — Upgrade for unlimited
                  </button>
                )}
              </div>
            )}
...
export default function MassIQ() {
    ...
    const hydrate = async () => {
      ...
      try {
        ...
        try {
          ...
          if (mounted) {
            setEntitlements(ent);
            if (ent != null && Number.isFinite(ent.free_food_scans_used)) {
              setFoodScanCache(userId, ent.free_food_scans_used);
            }
          }
  ...
  const renderTab = () => {
    const content = (() => {
      switch (tab) {
        case 'home':      return <HomeTab profile={profile} activePlan={activePlan} setTab={setTab} showToast={showToast} scanHistory={scanHistory} subscription={subscription} entitlements={entitlements} onUpgrade={() => setPaywallOpen(true)} userId={session?.user?.id} accessToken={session?.access_token} onFoodScanComplete={async () => { if (session?.access_token && session?.user?.id) { try { const ent = await getEntitlements(session.access_token, session.user.id); setEntitlements(ent); if (ent != null) setFoodScanCache(session.user.id, ent.free_food_scans_used ?? 0); } catch {} } }} />;
        case 'nutrition': return <NutritionTab profile={profile} activePlan={activePlan} showToast={showToast} setTab={setTab} subscription={subscription} onUpgrade={() => setPaywallOpen(true)} userId={session?.user?.id} accessToken={session?.access_token} entitlements={entitlements} onFoodScanComplete={async () => { if (session?.access_token && session?.user?.id) { try { const ent = await getEntitlements(session.access_token, session.user.id); setEntitlements(ent); if (ent != null) setFoodScanCache(session.user.id, ent.free_food_scans_used ?? 0); } catch {} } }} />;
        ...
        <Paywall
          ...
          onRefreshPremium={async () => {
            ...
            const [sub, ent] = await Promise.all([
              getSubscription(session.access_token, session.user.id),
              getEntitlements(session.access_token, session.user.id),
            ]);
            setSubscription(sub);
            setEntitlements(ent);
            if (ent != null && Number.isFinite(ent.free_food_scans_used)) {
              setFoodScanCache(session.user.id, ent.free_food_scans_used);
            }
          }}
        />
      )}
