...
function HomeTab({ profile, activePlan, setTab, showToast, scanHistory, subscription, entitlements, onUpgrade, userId, accessToken, onFoodScanComplete }) {
  ...
  const handleScanFile = (file) => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        showToast?.(String(err?.message || '').includes('NOT_FOOD') ? 'Oops, you need to scan a food.' : 'Scan failed — try again');
      } finally {
        setScanning(false);
      }
    };
  ...
  const confirmScan = () => {
    ...
    const prevUsed = getFoodScansUsedTotal(entitlements, userId);
    setFoodScanCache(userId, prevUsed + 1);
    onFoodScanComplete?.();
    setScanResult(null);
  ...
  const analyzePhoto = (file) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        setError(String(err?.message || '').includes('NOT_FOOD') ? 'Oops, you need to scan a food.' : 'Photo analysis failed — fill in manually.');
      } finally {
        setAnalyzing(false);
      }
    };
