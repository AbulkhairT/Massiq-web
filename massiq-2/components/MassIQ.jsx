...
function HomeTab({ profile, activePlan, setTab, showToast, scanHistory, subscription, entitlements, onUpgrade, userId, accessToken, onFoodScanComplete }) {
  ...
  const handleScanFile = (file) => {
    ...
    if (!canFoodScan(subscription, entitlements, userId)) {
      showToast?.(`Food scan limit reached (${FREE_FOOD_SCAN_LIMIT} per day). Upgrade for unlimited scans.`);
      return;
    }
    setScanning(true);
  ...
  return (
    <div className="screen" style={{ paddingBottom: 120 }}>
      ...
      {(() => {
        const foodRemaining = foodScansRemainingToday(subscription, entitlements, userId);
        const foodUsed      = getFoodScansUsedToday(entitlements, userId);
...
function LogMealModal({ onClose, onAdd, macros, profile, subscription, onUpgrade, userId, accessToken, entitlements, onFoodScanComplete }) {
  ...
  const analyzePhoto = (file) => {
    ...
    if (!canFoodScan(subscription, entitlements, userId)) {
      setError(`Food scan limit reached (${FREE_FOOD_SCAN_LIMIT} per day). Upgrade for unlimited scans.`);
      onUpgrade?.();
      return;
    }
    setAnalyzing(true); setError('');
            ...
            {aiTab === 'describe' ? (
              ...
              <div>
                {!isPremiumActive(subscription) && (() => {
                  const left = foodScansRemainingToday(subscription, entitlements, userId);
                  return (
      ...
      <div className="screen" style={{ gap: 18 }}>
        ...
        {result.weeklyMissions?.length > 0 && (
          <Card className="su" style={{ animationDelay: '.08s' }}>
            ...
            {premium ? <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {result.photoQuality && (
          <Card className="su" style={{ animationDelay: '.085s' }}>
            ...
            {premium ? <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: result.photoQuality.notes ? 10 : 0 }}>
              {[
            ...
            {premium && result.photoQuality.notes && (
    ...
    <div className="screen">
      ...
      {scanHistory.length > 0 && (
        <div style={{ marginTop: 8 }}>
          ...
          {premium ? <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
