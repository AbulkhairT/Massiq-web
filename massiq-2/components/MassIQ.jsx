...
function Onboarding({ onComplete, userId }) {
  ...
  useEffect(() => {
    ...
    if (!saved) return;
    if (saved.id && saved.id !== userId) return;
...
/* ─── Home Tab ───────────────────────────────────────────────────────────── */
function HomeTab({ profile, activePlan, scanHistory, setTab, showToast }) {
...
function PlanTab({ profile, activePlan, scanHistory, planStatus, planError, setTab, showToast }) {
  ...
  if (planStatus === 'loading' || planStatus === 'hydrating') {
    return (
      <div className="screen">
        <div style={{ background: C.card, borderRadius: 20, padding: '30px 24px', border: `1px solid ${C.border}`, textAlign: 'center' }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: C.white, marginBottom: 8 }}>Loading your plan…</div>
          <div style={{ fontSize: 14, color: C.muted }}>Syncing your latest scan and targets from your account.</div>
        </div>
      </div>
    );
  }
  ...
  if (planStatus === 'generating') {
    return (
      <div className="screen">
        <div style={{ background: C.card, borderRadius: 20, padding: '30px 24px', border: `1px solid ${C.border}`, textAlign: 'center' }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: C.white, marginBottom: 8 }}>Generating your scan-based plan…</div>
          <div style={{ fontSize: 14, color: C.muted }}>Your scan has been saved. We are finalizing your latest plan.</div>
        </div>
      </div>
    );
  }
  ...
  if (planStatus === 'error') {
    return (
      <div className="screen">
        <div style={{ background: C.card, borderRadius: 20, padding: '30px 24px', border: `1px solid ${C.border}`, textAlign: 'center' }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: C.white, marginBottom: 8 }}>Plan unavailable</div>
          <div style={{ fontSize: 14, color: C.muted, lineHeight: 1.65, maxWidth: 320, margin: '0 auto 20px' }}>
            {planError || 'We found a scan but could not load a valid plan. Run a new scan to regenerate your program.'}
          </div>
          <button className="bp" onClick={() => setTab('scan')} style={{ background: C.green, color: '#0A0D0A', border: 'none', padding: '12px 20px', borderRadius: 99, fontSize: 14, fontWeight: 700 }}>
            Go to Scan
          </button>
        </div>
      </div>
  ...
  /* ── No active plan ── */
  if (planStatus === 'no_scan' || !activePlan) {
    return (
      <div className="screen">
...
function ProfileTab({ profile, activePlan, scanHistory, setTab, onEditProfile, onReset, onLogout, showToast, onUpdateUnits }) {
  // Always display in chronological order (oldest first)
  const orderedHistory = [...(scanHistory || [])].sort((a, b) => new Date(a.date) - new Date(b.date));
  ...
  const lastScan    = orderedHistory[orderedHistory.length - 1];
          ...
          <Card style={{ padding: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 10, overflowX: 'auto', paddingBottom: 8, marginBottom: 14 }}>
              {orderedHistory.map((s, i) => {
                ...
                const isLatestScan = i === orderedHistory.length - 1;
                return (
            ...
            <div style={{ padding: '4px 0' }}>
              <PhysiqueChart scans={orderedHistory} />
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: C.dimmed, marginTop: 4 }}>
              <span>{fmt.date(orderedHistory[0]?.date)}</span>
              <span style={{ color: C.muted }}>Physique Score</span>
              <span>{fmt.date(orderedHistory[orderedHistory.length - 1]?.date)}</span>
            </div>
          </Card>
        )}
      </div>}
...
function ScanTab({ profile, scanHistory: initialHistory, setTab, showToast, onPlanApplied }) {
...
{"bodyFatRange":{"low":0,"high":0,"midpoint":0},"bodyFatConfidence":"medium","bodyFatReasoning":"specific visual markers that led to this range","leanMass":0,"leanMassTrend":"maintaining","physiqueScore":0,"symmetryScore":0,"symmetryDetails":"specific description of balance or imbalances","muscleGroups":{"chest":"moderate","shoulders":"moderate","back":"moderate","arms":"moderate","core":"moderate","legs":"moderate"},"weakestGroups":[],"limitingFactor":"the single most important thing holding this physique back","limitingFactorExplanation":"specific explanation with reference to their stats and what is visible","strengths":[],"asymmetries":[],"bodyFatSummary":"","muscleSummary":"","priorityAreas":[],"balanceNote":"","diagnosis":"2-3 sentence honest assessment referencing their specific stats","photoQualityIssues":[],"photoQuality":{"overall":"medium","lighting":"good","clothing":"acceptable","pose":"acceptable","notes":""},"recommendation":"2-3 sentence specific recommendation referencing their weight and goal","disclaimer":"Visual AI estimate based on photo. Accuracy improves with consistent lighting and front/side pose."}` },
  ...
  const applyPlan = async () => {
    ...
    const persisted = await onPlanApplied(plan, history, entry);
    if (!persisted) {
      showToast('Scan saved locally, but cloud sync is still in progress.');
      return;
    }
...
export default function MassIQ() {
  ...
  const [planStatus, setPlanStatus] = useState('hydrating');
  ...
  const hasValidScan = (scan) => {
    if (!scan) return false;
    return (scan.bodyFat != null || scan.bodyFatPct != null) && scan.leanMass != null;
  };

  const derivePremiumActive = (user, loadedProfile) => {
    return Boolean(
      user?.app_metadata?.premium_active
      || user?.user_metadata?.premium_active
      || loadedProfile?.premium_active
  ...
  useEffect(() => {
    ...
    const hydrate = async () => {
      ...
      try {
        ...
        const switchedUser = userIdRef.current && userIdRef.current !== userId;
        if (switchedUser) clearLocalMassiqKeys();
        ...
        const latestValidScan = [...loadedScanHistory].reverse().find(hasValidScan) || null;
        if (latestValidScan) {
          try {
        ...
        if (mounted) {
          // Restore name if DB doesn't have it.
          if (loadedProfile && !loadedProfile.name) {
            const persistedName = localStorage.getItem(`miq:name:${userId}`);
            if (persistedName) loadedProfile = { ...loadedProfile, name: persistedName };
          }
          setProfile(loadedProfile);
          setScanHistory(loadedScanHistory);
          if (latestValidScan && loadedPlan) {
            setActivePlan(loadedPlan);
            setPlanStatus('ready');
          } else if (latestValidScan && !loadedPlan) {
            setActivePlan(null);
            setPlanStatus('error');
            setPlanError('Scan exists but no valid plan was found. Please run a new scan to regenerate your plan.');
          } else {
            setActivePlan(null);
            setPlanStatus('no_scan');
          }
          setTab('home');
          LS.set(LS_KEYS.profile, loadedProfile);
          if (loadedPlan && latestValidScan) LS.set(LS_KEYS.activePlan, loadedPlan);
          else localStorage.removeItem(LS.scopedKey(LS_KEYS.activePlan));
        ...
        if (mounted) {
          setAuthError('');
          setPlanStatus('error');
          setPlanError('Could not hydrate your account data.');
        }
      } finally {
  ...
  const persistUserProfile = async (nextProfile) => {
  ...
  const persistScanAndPlan = async (nextPlan, nextHistory, latestScanEntry) => {
    ...
    try {
      ...
      const userId = user?.id;
      if (!userId) throw new Error('Missing user session');
      const scanPayload = latestScanEntry || nextHistory?.[nextHistory.length - 1];
      if (!scanPayload || !hasValidScan(scanPayload)) throw new Error('Missing valid scan payload');
      await createScan(session.access_token, userId, scanPayload);
      await upsertPlan(session.access_token, userId, nextPlan);
      const [freshScans, freshPlan] = await Promise.all([
        getScans(session.access_token, userId),
        getPlan(session.access_token, userId),
      ]);
      const freshLatest = [...freshScans].reverse().find(hasValidScan) || null;
      if (!freshLatest || !freshPlan) throw new Error('Missing scan-backed plan after sync');
      setScanHistory(freshScans);
      setActivePlan(freshPlan);
      setPlanStatus('ready');
      LS.set(LS_KEYS.scanHistory, freshScans);
      LS.set(LS_KEYS.activePlan, freshPlan);
      return true;
    } catch (err) {
      console.error('[sync] scan/plan persistence failed', err);
      setPlanStatus('error');
      setPlanError('Your scan was captured, but we could not confirm a scan-backed plan from the server.');
      return false;
    } finally {
      setSyncing(false);
    }
  ...
  const handleOnboardingComplete = (p) => {
    ...
    setPlanStatus('no_scan');
    setPlanError('');
    localStorage.removeItem(LS.scopedKey(LS_KEYS.activePlan));
        ...
        const poll = async () => {
          while (attempts < maxAttempts) {
            ...
            try {
              ...
              const active = derivePremiumActive(freshUser, profile);
              if (active) {
          ...
          showToast('Premium activation is still processing. Refresh again in a moment.');
        };
        poll();
      }
    }
  ...
  if (!profileComplete || editing) return (
    <>
      <style>{CSS}</style>
      <Onboarding key={`ob-${session?.user?.id || 'anon'}`} onComplete={handleOnboardingComplete} userId={session?.user?.id || null} />
    </>
  );
  ...
  const renderTab = () => {
    const content = (() => {
      switch (tab) {
        case 'home':      return <HomeTab profile={profile} activePlan={activePlan} scanHistory={scanHistory} setTab={setTab} showToast={showToast} />;
        case 'nutrition': return <NutritionTab profile={profile} activePlan={activePlan} showToast={showToast} setTab={setTab} />;
        case 'scan':      return <ScanTab key={`scan-${session?.user?.id || 'anon'}`} profile={profile} scanHistory={scanHistory} setTab={setTab} showToast={showToast} onPlanApplied={persistScanAndPlan} />;
        case 'plan':      return <PlanTab profile={profile} activePlan={activePlan} scanHistory={scanHistory} planStatus={planStatus} planError={planError} setTab={setTab} showToast={showToast} />;
        case 'profile':   return (
          <ProfileTab
            profile={profile}
