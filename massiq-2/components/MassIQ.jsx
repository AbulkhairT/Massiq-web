...
import { buildFoodTrendSummary } from '../lib/engine/foodSignals';
import {
  initializeSession,
  getStoredSession,
  signInWithPassword,
  ...
  const persistUserState = async (nextProfile, nextPlan, newScanEntry = null, { criticalProfile = false, previousPlan = null } = {}) => {
    ...
    try {
      ...
      if (savedScanEntry?.dbId) {
        ...
        try {
          const rawSignals = extractBodySignals(savedScanEntry);
          console.info('[scan-signals] raw extracted signals', rawSignals);
          await createScanSignalSet(token, userId, { scanId: savedScanEntry.dbId, ...rawSignals });
          const previousState = await getUserBodyState(token, userId);
          const recentSignals = await getRecentScanSignalSets(token, userId, 3);
          const stabilizedState = stabilizeBodyState({
            previousState,
            previousSignals: recentSignals,
            currentSignals: rawSignals,
            scanId: savedScanEntry.dbId,
            previousScanId: prevIdForCompare || null,
          });
          await upsertUserBodyState(token, userId, stabilizedState);
          stableComparison = buildStableComparison({ previousState, currentState: stabilizedState });
          console.info('[scan-stable] comparison', stableComparison);

          const recentFoodRows = await getFoodLogsRecentForAdherence(token, userId, 30);
          const foodTrend = buildFoodTrendSummary(recentFoodRows);
          const stableDecisionInput = buildDecisionEngineInput({
            profile: nextProfile,
            currentPlan: nextPlan,
            stabilizedBodyState: stabilizedState,
            latestSignals: rawSignals,
            recentFoodSummary: foodTrend,
          ...
          console.error('[scan-stable] pipeline failed, fallback to legacy decision path', stableErr?.message, stableErr);
        }
        ...
        if (prevIdForCompare) {
          try {
            await createScanComparison(token, userId, {
              ...
              bodyFatDelta: stableComparison?.bodyFatDelta ?? (currBF != null && prevBF != null ? Number((currBF - prevBF).toFixed(2)) : null),
              leanMassDelta: stableComparison?.leanMassDeltaKg ?? (
                savedScanEntry.leanMass != null && priorScan.leanMass != null
                  ? Number((((savedScanEntry.leanMass - priorScan.leanMass) * 0.453592)).toFixed(3))
                  : null
              ),
              physiqueScoreDelta:
              ...
              symmetryScoreDelta:
                savedScanEntry.symmetryScore != null && priorScan.symmetryScore != null
                  ...
                  : null,
              weightDelta,
              summary: stableComparison?.summary || savedScanEntry.adaptationRationale || null,
              comparisonConfidence: stableComparison?.comparisonConfidence || savedScanEntry.confidence || null,
              improvedAreas: stableComparison?.improvedAreas || improvedAreas,
              worsenedAreas: stableComparison?.worsenedAreas || worsenedAreas,
            });
          } catch (cmpErr) {
