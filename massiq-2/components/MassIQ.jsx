...
import { hasFeature, isPremiumActive, FEATURES, canScan, scansRemaining, FREE_SCAN_LIMIT, canFoodScan, foodScansRemainingToday, recordFoodScanUsed, FREE_FOOD_SCANS_PER_DAY } from '../lib/features';
...
function HomeTab({ profile, activePlan, setTab, showToast, scanHistory, subscription, entitlements, onUpgrade }) {
  ...
  const confirmScan = () => {
    ...
    // Count this as a completed food scan (only on user confirm, not on failed/canceled scans)
    recordFoodScanUsed(profile?.id);
    setScanResult(null);
  ...
  return (
    <div className="screen" style={{ paddingBottom: 120 }}>
      ...
      {(() => {
        ...
        const foodLeft = foodScansRemainingToday(userId, subscription);
        const blocked  = !isPremiumActive(subscription) && foodLeft <= 0;
        return (
          <div style={{ position: 'fixed', bottom: 88, left: '50%', transform: 'translateX(-50%)', zIndex: 50, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
            <button
              className="bp"
              onClick={() => {
              ...
              disabled={scanning}
              style={{
                height: 50, paddingInline: 28, borderRadius: 25,
                background: scanning ? C.greenDim : blocked ? 'rgba(255,255,255,0.08)' : C.green,
                border: blocked ? `1px solid rgba(255,255,255,0.15)` : 'none',
                color: blocked ? C.muted : '#0A0D0A', fontSize: 14, fontWeight: 700,
            ...
            >
              ...
              {scanning ? 'Scanning…' : blocked ? 'Scan limit reached' : 'Scan'}
            </button>
            {!isPremiumActive(subscription) && (
              <div style={{ fontSize: 11, color: foodLeft > 0 ? C.dimmed : '#ef4444', whiteSpace: 'nowrap' }}>
                {foodLeft > 0 ? `${foodLeft} food scan${foodLeft === 1 ? '' : 's'} left today` : '0 food scans left — Upgrade for unlimited'}
              </div>
            )}
          </div>
        );
...
/* Log Meal Modal */
function LogMealModal({ onClose, onAdd, macros, profile, subscription, userId, onUpgrade }) {
  ...
  const fileRef = useRef(null);
  // Tracks whether the current form data came from a photo scan (for counting purposes)
  const photoScannedRef = useRef(false);
  ...
  const analyzePhoto = (file) => {
    if (!file) return;
    // Enforce daily food-scan limit before hitting the API
    if (!canFoodScan(userId, subscription)) {
      setError('No food scans left today. Upgrade to Premium for unlimited scans.');
      return;
    }
    setAnalyzing(true); setError('');
    const reader = new FileReader();
  ...
  const handleAdd = () => {
    ...
    // Record food scan usage only when user confirms a photo-analyzed meal
    if (photoScannedRef.current) {
      recordFoodScanUsed(userId);
      photoScannedRef.current = false;
    }
    onAdd(safeMeal);
    onClose();
  };
          ...
          <div style={{ background: C.cardElevated, borderRadius: 16, padding: 16, marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', background: C.card, borderRadius: 10, padding: 3, marginBottom: 14 }}>
              {[['describe','Note Describe'],['photo','Photo Photo']].map(([k, lbl]) => (
                <button key={k} className="bp" onClick={() => { setAiTab(k); if (k !== 'photo') photoScannedRef.current = false; }} style={{
            ...
            {aiTab === 'describe' ? (
              ...
              <div>
                ...
                {canFoodScan(userId, subscription) ? (
                  <>
                    ...
                    {!isPremiumActive(subscription) && (() => {
                      const left = foodScansRemainingToday(userId, subscription);
                      return left < FREE_FOOD_SCANS_PER_DAY ? (
                        <div style={{ fontSize: 11, color: C.dimmed, textAlign: 'center', marginTop: 6 }}>
                          {left} food scan{left === 1 ? '' : 's'} left today
                        </div>
                      ) : (
                        <div style={{ fontSize: 11, color: C.dimmed, textAlign: 'center', marginTop: 6 }}>
                          {FREE_FOOD_SCANS_PER_DAY} food scans per day on free plan
                        </div>
                      );
                    })()}
                  </>
                  ...
                  <div style={{ textAlign: 'center', padding: '20px 0' }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: C.white, marginBottom: 4 }}>0 food scans left today</div>
                    <div style={{ fontSize: 12, color: C.muted, marginBottom: 14 }}>Free plan includes {FREE_FOOD_SCANS_PER_DAY} food scans per day. Resets at midnight.</div>
                    {onUpgrade && (
                      <button className="bp" onClick={() => { onClose(); onUpgrade(); }} style={{
                        background: C.green, color: '#0A0D0A', border: 'none',
                        padding: '10px 22px', borderRadius: 99, fontSize: 13, fontWeight: 700, cursor: 'pointer',
...
/* Main Nutrition Tab */
function NutritionTab({ profile, activePlan, showToast, setTab, subscription, userId, onUpgrade }) {
  const today = new Date().toISOString().slice(0, 10);
  const [meals,        setMeals]        = useState(() => LS.get(LS_KEYS.meals(today), []));
  const [showModal,    setShowModal]    = useState(false);
  ...
  const renderTab = () => {
    const content = (() => {
      switch (tab) {
        ...
        case 'nutrition': return <NutritionTab profile={profile} activePlan={activePlan} showToast={showToast} setTab={setTab} subscription={subscription} userId={session?.user?.id} onUpgrade={() => setPaywallOpen(true)} />;
    ...
    <>
      ...
      {paywallOpen && (
        <Paywall
          ...
          onPremiumActivated={(sub) => { setSubscription(sub); setPaywallOpen(false); }}
        />
      )}
    </>
  );
