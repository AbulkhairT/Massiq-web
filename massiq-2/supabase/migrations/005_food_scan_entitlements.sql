...
-- RPC: Increment only (no check). Call ONLY after successful food scan.
-- Used when Claude returns valid food data (not error:"not_food").
CREATE OR REPLACE FUNCTION public.record_food_scan(p_user_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  SET free_food_scans_used = public.user_entitlements.free_food_scans_used + 1,
      updated_at = now();
END;
$$;
...
-- Notes
-- =============================================================================
-- • allocate_food_scan: check+increment atomically (deprecated for food scans).
-- • record_food_scan: increment only. Call AFTER Claude succeeds with food data.
-- • /api/food-scan now: 1) check used<2, 2) call Claude, 3) if success+food then record_food_scan.
-- =============================================================================
